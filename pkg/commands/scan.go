...
func newScanCmd() *scanCmd {
	...
	cc.baseCmd = newBaseCmd(&cobra.Command{
		...
		Long:  ``,
		Example: `# Scan images by image list file.
hangar scan \
	--file IMAGE_LIST.txt \
	--arch amd64,arm64 \
	--os linux`,
	...
	flags.StringVarP(&cc.format, "format", "", "json", "report format (available: json/yaml/csv)")
...
func (cc *scanCmd) prepareHangar() (hangar.Hangar, error) {
	...
	if !slices.Contains(availableFormat, cc.format) {
		return nil, fmt.Errorf("invalid output format %q, available %v", cc.format, availableFormat)
	}
	if cc.debug {
...
func (cc *scanCmd) prepareScanner() error {
	...
	// Init global scanner.
	err = scan.InitScanner(scan.Option{
		TrivyServerURL:        cc.trivyServerURL,
		Offline:               cc.offline,
		InsecureSkipTLSVerify: !cc.tlsVerify,
		CacheDirectory:        cc.cacheDir,
...
func (cc *scanCmd) saveReport() error {
	...
	default:
		return fmt.Errorf("invalid output format: %v", cc.format)
	}
