...
func (s *Source) scanDockerV2ListMediaType(
	...
	for _, m := range s.schema2List.Manifests {
		...
		imageResult.Platform = scan.Platform{
			Arch:       arch,
			OS:         osInfo,
			Variant:    variant,
			OSVersion:  osVersion,
...
func scanImage(
	...
	return scan.Scan(ctx, o.referenceName)
}
